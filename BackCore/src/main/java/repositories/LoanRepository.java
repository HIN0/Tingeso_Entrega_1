...
import java.util.Set; // Importar Set para clientes únicos
...
public interface LoanRepository extends JpaRepository<LoanEntity, Long> {
    ...
    @Query("SELECT l.tool, COUNT(l) as total " +
           ...
           "GROUP BY l.tool " +
           "ORDER BY total DESC")
    List<Object[]> findTopToolsByDateRange(@Param("from") LocalDate from, @Param("to") LocalDate to); // Añadir @Param si no estaba


    // --- NUEVOS MÉTODOS CON FILTRO DE FECHA ---
    ...
    List<LoanEntity> findByStatusAndStartDateBetween(@Param("status") LoanStatus status,
                                                   @Param("from") LocalDate from,
                                                   @Param("to") LocalDate to);

    // RF6.2: Clientes únicos con préstamos ATRASADOS que INICIARON en un rango de fechas
    // Nota: Podríamos filtrar por fecha de vencimiento (dueDate) si fuera más lógico. Usaremos startDate por consistencia.
    @Query("SELECT DISTINCT l.client FROM LoanEntity l WHERE l.status = :status AND l.startDate BETWEEN :from AND :to")
    List<ClientEntity> findDistinctClientsByStatusAndStartDateBetween(@Param("status") LoanStatus status,
                                                                    @Param("from") LocalDate from,
                                                                    @Param("to") LocalDate to);
